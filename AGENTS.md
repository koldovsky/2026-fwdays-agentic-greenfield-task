...
([openspec/backlog.md](./openspec/backlog.md)) and driven by the **ship-change** skill (`/ship-change`)
...
rule #12), + the evals from [ADR-0013](./docs/adr/0013-eval-framework.md)), and marks `blocked` on
failure then continues to the next ready change. The skill splits into a generic `SKILL.md` + a
repo-specific `PROFILE.md`, and delegates each phase to a named agent under `.claude/agents/`
(`ship-maker`/`-verifier`/`-arch`/`-reviewer`/`-gates`/`-docs`) so maker ≠ checker is a distinct
subagent. See [ADR-0012](./docs/adr/0012-implementation-loop-runner.md) for its full shape.
