...
**mytv** — a self-hosted Samsung TV remote for a local network. Runs on an Orange Pi, advertises itself as `mytv.local` over mDNS, discovers Samsung Smart TVs over UPnP, and controls them via Samsung's IP Control protocol (JSON-RPC 2.0 over HTTPS). The Fastify back-end serves the compiled React SPA and the HTTP/WebSocket API from a **single origin** (`http://mytv.local/`), so there are no CORS or cross-port concerns in production. Two screens: **device list** and **remote**. No cloud, no accounts, no internet required at runtime. Full brief in `docs/product-brief.md`.

## Repo layout

| Path                                  | What lives there                                                                                                                                          |
| ------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `back-end/`                           | Fastify 5 + TypeScript service. Discovery, IP Control client, HTTP API and WebSocket for the front-end, **and static hosting of the built SPA** (serves `front-end/dist/` in production; SPA-fallback to `index.html`). |
...
| `docs/samsung-ip-control-protocol/`   | Samsung HTV IP Control v2.8 PDF spec + Postman collection. Ground truth for wire format.                                                                  |
...
- **`samsung-ip-control-protocol`** — load when editing `back-end/**` code that talks to a TV, encodes/decodes JSON-RPC, or handles discovery/errors. Covers ports (1515/1516), TLS caveats, method catalog, error codes, connection guardrails. Do **not** load for front-end or docs work.
...
### Security & privacy
- **Never send the TV `AccessToken` to the front-end.** It stays in `back-end/` config + logs-with-redaction. If you catch yourself passing it through an API response, stop.
- **No cloud services, no analytics, no telemetry.** The product principle is local-first — anything that dials home is a bug.
- Log the JSON-RPC `id` on request and response for correlation; strip `AccessToken` from every log line.

### Back-end
- Follow the `samsung-ip-control-protocol` skill for anything protocol-adjacent — do not re-derive port numbers or error codes from the PDF each time.
- One keep-alive HTTPS agent per TV; serialize state-changing commands per TV; do not batch JSON-RPC (spec explicitly says batch is not guaranteed).
- Use `UDN` (uuid from UPnP description) as the stable TV identity, not IP.
- Map TV errors to a domain error union (`TvNotReachable | TvNotSupported | TvFailed | TvInvalidOp | TvUnknown`). Never leak raw `-32xxx` codes to the UI.
...
- When adding a route/handler, first check the Postman collection at `docs/samsung-ip-control-protocol/IP CONTROL V2.postman_collection.json` for the exact `params` shape before writing types.
