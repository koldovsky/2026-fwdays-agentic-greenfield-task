# agentic-crash-course — Agent Rules

> Stack-specific placeholders below read **TBD (set at Phase 1 stack decision)** —
> init is stack-agnostic, so the framework/DB are filled when the stack is chosen
> and scaffolded (recorded as an ADR under `docs/adr/`).

# This may NOT be the framework you know

The installed framework (**TBD — set at Phase 1**, version **TBD**) may differ
from training data. Read the relevant guide in `node_modules/<framework-pkg>/dist/docs/`
(or the package's bundled docs) before writing any code. Heed deprecation notices.
...
an ADR is accepted. Write last update date/time (timezone: Europe/Kiev) and
...
> The conventions above assume a TS/Next-style stack as a default. Revise this
> whole section to match the actual stack chosen at Phase 1.
...
- Host: macOS (Darwin 25.5.0), zsh shell, Node v26. Timezone Europe/Kiev (EEST).
  Note: zsh does NOT word-split unquoted variables — quote or loop in scripts.
- Database: TBD — set at Phase 1 stack decision.
