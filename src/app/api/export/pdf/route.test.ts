...
describe("POST /api/export/pdf — server-side paywall (FR-PAYWALL-01)", () => {
  ...
  it("renders for a paid caller", async () => {
    ...
    expect(renderResumePdf).toHaveBeenCalledWith(bulletlessDoc);
  });
...
describe("POST /api/export/pdf — server-side membership gate (BC-HONESTY-02, NFR-SEC-04)", () => {
  ...
  it("tailoringId absent but empty (non-string, e.g. a number): 400 missing_tailoring on a bullet-bearing doc", async () => {
