...
## Purpose
Email + password authentication and the per-user boundary for Cadence: accounts, server-side session
cookies, the current-user dependency, CSRF protection, and per-user data isolation. Requirement text
is authoritative in `docs/requirements.md`; each requirement below cites its stable FR/NFR id. Built
by slice 001 (recorded via change `add-auth-email`). OAuth sign-in (FR-AUTH-04/05) is slice 009.
