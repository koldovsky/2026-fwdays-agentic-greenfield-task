...
The PDF format handler is an install-on-demand first-party plugin (`format.pdf`) that opens PDF files
into a `Publication` with a page-based reading order, served as rasterized canvas frames — not as
origin-isolated HTML/JS execution. This is intentionally asymmetric to EPUB: a PDF file renders
inert pixels, not book-authored scripts, so it can safely render in-app. pdfjs-dist's low-level API
(`getDocument` + `getPage` + `pdfPage.render({canvas})`) is used directly, hardened with
`isEvalSupported: false` and `enableScripting: false` (defense-in-depth against CVE-2024-4367 class
vulnerabilities). Book bytes are served by range reads through a custom pdfjs transport wired to
`PublicationSource.read`, bypassing the Service Worker (ADR-005) and never requiring the full file in
memory. Sourced from change `add-extensions-and-capability-install`.
...
### Requirement: In-app canvas rendering with eval and scripting lockdown

The PDF handler SHALL render pages to an in-app `<canvas>` element using the pdfjs low-level API
(`getDocument` + `getPage` + `pdfPage.render({canvas})`). This design is intentionally different from
EPUB origin isolation: PDF renders rasterized pixels and executes no book-authored HTML or JavaScript,
so origin isolation is not required. The handler SHALL harden the parse by setting
`isEvalSupported: false` (preventing pdfjs's eval-based fast paths from reaching untrusted document
bytes) and `enableScripting: false` (keeping the PDF's own OpenAction/field scripts inert). No
`cMapUrl`, CDN fetch, or network access is used during rendering.

#### Scenario: Canvas render is in-app, not origin-isolated

- **WHEN** a PDF page is rendered
- **THEN** it is drawn to an in-app `<canvas>` element, not in a cross-origin iframe

#### Scenario: Eval and scripting are disabled

- **WHEN** the PDF handler opens a document with pdfjs
- **THEN** `isEvalSupported: false` and `enableScripting: false` are passed to `getDocument`, preventing
  document bytes from reaching `eval`/`new Function` and keeping document scripts inert
