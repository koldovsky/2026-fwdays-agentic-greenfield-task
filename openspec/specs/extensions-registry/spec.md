# Spec: Extensions Registry (UI)

## Purpose

The Extensions settings screen (`/settings/extensions`) gives users a visible catalogue of every
installed and available plugin. It maps directly onto the `PluginRegistry` state: the INSTALLED section
lists enabled plugins with their identity, capability chips, and a "BUNDLED" tag for always-on plugins;
the AVAILABLE section lists catalog entries not yet enabled, each with an Install action. The screen
reflects and drives the registry — install, enable, and disable actions call through to the registry
and persist. Bundled plugins render their toggle as non-interactive, mirroring the registry's
`PluginNotDisableableError` enforcement. The host API pill ("Host API v1.2") reflects the running
`HOST_API_VERSION`. Sourced from change `add-extensions-and-capability-install`.
...
INSTALLED section (enabled), and the choice SHALL persist across reloads. Kavita, Calibre, and CBZ are
catalog stubs in this change: clicking Install surfaces "Coming soon" via `PluginNotAvailableYetError`
...
#### Scenario: Stub catalog entries surface "Coming soon"

- **WHEN** the user clicks Install on Kavita, Calibre, or CBZ comics
- **THEN** the UI shows a "Coming soon" message and the plugin remains in the AVAILABLE section with
...
reversible and SHALL NOT delete the plugin or its data. Bundled plugins (OPDS, Komga, EPUB) SHALL NOT
be disableable: the registry throws `PluginNotDisableableError` and the UI renders their toggle
non-interactive, so `resolveFormat(EPUB)` can never flip to `installable` and break the reader.

#### Scenario: Disabling an extension removes it from resolution

- **WHEN** the user toggles an installed non-bundled format off
...
#### Scenario: Bundled plugins cannot be disabled

- **WHEN** the user views the enable toggle for a bundled plugin (OPDS, Komga, or EPUB)
- **THEN** the toggle is rendered non-interactive and the registry refuses any attempt to disable it
  (throws `PluginNotDisableableError`), ensuring the core reader and OPDS fallback always resolve
...
that every plugin receives only the `HostBridge` (`DESIGN-CONNECTORS.md` §11).
