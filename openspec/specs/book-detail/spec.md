...
The Book Detail screen (screen 02, `doc/web/02-book-detail-desktop.png`) is the presentation surface
a reader lands on after clicking a Library card. It composes data owned by other capabilities —
`BookMeta` from `Connector.getBook`, the current-format `Locator` from the sync/progress store, and
`Publication.toc` from a format handler — and renders them in a two-column layout: cover + action
stack on the left, identity/metadata/progress cards/chapters on the right. It also surfaces Edda's
load-bearing per-`(sourceId, bookId, mediaType)` progress-keying invariant as a user-visible explainer
card. Sourced from change `add-book-detail`.
