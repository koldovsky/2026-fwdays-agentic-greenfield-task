# composite-dish Specification

## Purpose
Let a user save a recurring multi-item dish (e.g. a protein cocktail = whey + milk, coffee with
cream) once as a single named `food_database` product — basis `portion`, macros summed in code from
the dish's `food_log` rows — and re-log the whole dish by name later as a `fact`, quantity-scaled,
with no new lookup path and zero LLM calls.
