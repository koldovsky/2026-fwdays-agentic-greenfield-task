...
The Add-a-source flow lets the user connect a new library server or OPDS feed to Edda. A 3-step
native `<dialog>` modal (Address → Detected → Sign in) probes the entered URL via the
`server-prober`, surfaces the detected server kind and its capability chips, collects optional
credentials, and persists the source on-device. After connecting, the Library immediately renders
books from the newly connected source. The flow is also the entry point for the bare-OPDS and
installable-connector paths.
