...
First post-MVP capability (C10, Phase 5): open a user-supplied URL in the TV's built-in Tizen web browser from the SPA. Introduces a new Samsung Smart View WebSocket envelope family — `ms.channel.emit` with an `ed.apps.launch` payload targeting `org.tizen.browser` — that is orthogonal to the `ms.remote.control` traffic used by remote-control-keys (C6), volume-control (C7), and input-management (C8). Consumes the per-TV session queue from `tv-connection-lifecycle` (C5) and the shared toast pattern from `error-surfacing` (C9) verbatim; no transport, session-manager, or DS-primitive changes required. Establishes the template for future `ed.apps.launch`-based capabilities (YouTube, Netflix, etc.) once the envelope is verified on real hardware.
