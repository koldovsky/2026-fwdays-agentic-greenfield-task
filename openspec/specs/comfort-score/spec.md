...
Defines the comfort scoring capability: a pure function that computes a 0–100 comfort score for a forecast day, the badge UI displayed on each day card, and the weekend comfort summary banner.
...
Each day card in the 7-day forecast grid SHALL display a colored comfort badge showing the numeric score. Badge color SHALL follow the thresholds: green (`--comfort-good-*` tokens) for score ≥ 70, yellow (`--comfort-fair-*` tokens) for score 40–69, red (`--comfort-poor-*` tokens) for score < 40. The badge SHALL have an accessible `aria-label` in Ukrainian, e.g. "Комфорт: 82".
...
- **THEN** the badge renders with the yellow design-system tokens (`--comfort-fair-bg`, `--comfort-fair-text`)
