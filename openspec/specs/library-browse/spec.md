...
The Library Browse screen is the catalog view the user lands on when at least one Connector is bound.
It renders the connector's `browse()` result as two sections — a horizontal "Keep reading" row of
in-progress cards and a "Recently added" grid/list of covers — with a header carrying the catalog
counts, a local search box, and a sync-status pill. All catalog logic depends only on the `Connector`
interface from `core/contracts`, making any conforming connector substitutable with no Library changes.
Sourced from change `add-library-browse`.
