...
Second Phase-6 capability (C12), follow-up to C11 (`systemd-installer`). Adds `scripts/build.sh` — a no-root, no-systemd Bash script that runs `npm run install:all`, `back:build`, and `front:build` and asserts `back-end/dist/index.js` and `front-end/dist/index.html` afterwards. Runnable on macOS or Linux (build artefacts are pure JS and portable across architectures). Feeds C11 which no longer builds anything itself — the two capabilities together form the two-step deploy story (`./scripts/build.sh` → `sudo ./scripts/install.sh`), supporting both single-host (build + install on the Pi) and split-host (build on dev laptop / CI, `rsync` dists + scripts to Pi, install) workflows. Serves `FR-BUILD-01`.
