...
The guided, multi-step tailoring flow: Analyze, Confirm, Clarify, Generate,
Export. Analysis (parse, extract, score) runs and pauses before any bullet is
generated, so the user always sees the match score and checklist before
committing to generation. Weak requirements surface as a small set of
deterministic, non-blocking clarifying questions whose confirmed answers
become tagged grounding evidence alongside the CV. The flow ends in exporting
the tailored résumé — copy to clipboard, PDF, or DOCX — from one shared
export-document model, with a free-tier attribution footer applied by
default. Traces: FR-WIZARD-01/02/03/04/05, FR-EXPORT-01/02/03/04,
BC-HONESTY-03.
...
#### Scenario: Non-paid caller cannot bypass the client to export
- **WHEN** a non-paid or anonymous caller sends a PDF or DOCX export request directly to the server, bypassing the UI
- **THEN** the server rejects the request before rendering, rather than trusting the client to have withheld it
