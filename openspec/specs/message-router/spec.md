...
The FR-1 spine (requirements §8.0): every non-command inbound message is first classified by one
deterministic Sonnet call into one of six intents, with structured fields and a resolved calendar
date (user timezone). It sends no chat history to the model — the DB is the memory (invariant #1).
