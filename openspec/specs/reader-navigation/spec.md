# Capability: reader-navigation

## Purpose

The Reader screen mounts a foliate-js EPUB renderer (via `format-epub`) inside a cross-origin
isolation frame (via `reader-isolation`), drives paging and navigation, tracks the reading position,
and hands locator updates to the sync engine. It owns the full-screen reader chrome: top bar (back,
title/author, TOC / bookmark / Aa controls), two-page parchment spread, edge chevrons, keyboard
shortcuts, and the bottom position bar.
