...
Correcting a logged entry: when the router classifies an inbound message as `correction`, the
system updates the acting user's most recent `food_log` row in place — never inserting a new row
— and confirms the corrected value. Corrected macro/kcal numbers are always computed in code from
the entry's own basis, exactly like the logging path (invariant #2), never emitted by the model. A
quantity-only correction issues zero LLM calls; a product correction issues at most one structured
LLM call and runs no agent loop (invariant #5). The correction is always resolved and applied
through the tenancy helpers so another user's rows can never be touched (invariant #8), and the
confirmation prose mirrors the user's language while DB fields and enum literals stay English
(invariant #6).
