...
The design system defines the visual language and reusable UI primitives for Edda's app chrome.
It establishes the warm "parchment" color palette as named tokens, a typographic scale using
self-hosted serif and monospace faces, and a set of composable chrome primitives (buttons, chips,
cards, progress bars, etc.) that all screens compose rather than invent ad hoc. Sourced from
