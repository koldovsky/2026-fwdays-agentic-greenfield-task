...
Enables users to search for a city by name and select it as the active location. The feature provides a debounced combobox input that calls an internal geocoding Route Handler, displays suggestion rows with city metadata, updates the URL on selection, and meets full keyboard and screen-reader accessibility requirements.
