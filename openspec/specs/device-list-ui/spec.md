...
Render the live device registry produced by `upnp-tv-discovery` as the SPA's landing screen. Subscribes to the `/ws` `devices` topic for real-time updates, falls back to `GET /api/devices` if the WebSocket is slow, and shows a friendly empty state when nothing has been discovered yet. Built entirely from Orbit design-system primitives + tokens — no ad-hoc palette, shadow strings, or `@ds/ui_kits/**` imports.
