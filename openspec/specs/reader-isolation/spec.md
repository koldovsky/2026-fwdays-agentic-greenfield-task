# Capability: reader-isolation

## Purpose

Untrusted book content (EPUB HTML/JS) renders on a separate browser origin so it physically cannot
reach the app origin's credentials (`edda.creds.*` in `localStorage`). The isolation mechanism is a
cross-origin `<iframe>` served from a configurable reader origin, driven by an app↔frame
`MessageChannel` bridge. Book bytes cross as a Transferable `ArrayBuffer`; the foliate engine and all
spine documents live inside the isolated frame.
