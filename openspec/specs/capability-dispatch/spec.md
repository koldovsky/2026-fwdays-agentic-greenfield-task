# Spec: Capability Dispatch

## Purpose

The `CapabilityDispatcher` resolves an attempt to open a resource into exactly one of three statuses —
`ready`, `installable`, or `unsupported` — by sniffing the resource's media type and asking the plugin
registry to resolve a matching, installed-and-enabled format handler. On an `installable` result it
publishes a `CapabilityMissing` event over the host event bus; one or more UI subscribers drive the
install prompt. The dispatcher is stateless and platform-neutral — it contains no DOM, `fetch`, or
`window` usage so the native client can re-express it verbatim. Sourced from change
...
- **AND** the dispatcher re-resolves the PDF to `ready` and the book opens (`DESIGN-CONNECTORS.md` §8.1 sequence)
