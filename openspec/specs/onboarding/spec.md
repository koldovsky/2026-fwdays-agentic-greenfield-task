...
The first-run flow (US-1, M2): `/start` walks a new user through a short Q&A — age, sex, height,
weight, goal, activity, timezone — and computes daily calorie/macro targets via Mifflin–St Jeor in
code (no LLM). Progress is the DB itself (ADR-0016): each answer persists as it arrives and the next
question is the first still-null field, so onboarding resumes after a restart — no chat-history
state (invariant #1). Weight is recorded as the first `body_metrics` entry, not on `users`.
