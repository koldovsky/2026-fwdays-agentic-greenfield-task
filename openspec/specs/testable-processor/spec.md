# Capability: Testable Processor

## Purpose

Make the `process_expense` retry loop testable without an LLM or database by accepting injectable `extract_fn` and `validate_fn` callables, while preserving default behavior when none are provided.
