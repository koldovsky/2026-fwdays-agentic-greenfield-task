...
authenticated user, plus the notes list and trash views that surface them. Covers FR-020,
...
The system SHALL show an authenticated user a list of their own, non-deleted notes, with an
...
- **THEN** all of that user's notes with `deletedAt` unset are shown, ordered by most recently
  updated, and no other user's notes appear
