# daily-insight Specification

## Purpose

The one AI feature: a short, natural-language sentence about today's pace, generated
server-side from the last 14 local days and shown as a calm card on Stats. The LLM (Anthropic)
is called only on the NestJS backend and receives a pre-computed numeric summary — never raw
rows. Output is guardrailed (≤ 200 chars, English, no emoji, no invented figures), cached one
per user per local day (cost bound), and always degrades to a deterministic, pure, unit-tested
fallback so the feature never errors or blanks. (FR-INSIGHT-01→06, NFR-COST-01, NFR-OBS-01,
TC-STACK-07, TC-PURE-01, TC-TEST-01)
