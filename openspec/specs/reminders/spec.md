...
## Purpose
This capability gives the single Owner an at-a-glance view of which plants need watering and lets them water a plant in one tap from the home view. It derives a per-plant watering status — **healthy / soon / overdue** — purely from each plant's `intervalDays` and its last watering date (the latest existing watering event), evaluated against today in Europe/Kiev. The home view summarizes how many plants are due today, lists urgency-ordered reminder rows with a "water now" action, and shows an all-done empty state when nothing is due; the status is also surfaced as a pill on plant cards and the plant detail view. The last watering date is DERIVED from the watering capability's events (the latest event's date) — this capability introduces NO duplicate "last watered" state. Push/OS notifications (FR-REM-08) are explicitly Future and intentionally unsupported in this baseline. No authentication or authorization applies: there is a single local Owner with no auth in MVP (NFR-SEC-01, TC-04, Q6), so unauthorized/forbidden paths are intentionally N/A.

### Scope and exclusions
- The watering interval `intervalDays` is the only NEW persisted field this capability adds to the plant model (FR-REM-01). All other inputs — the last watering date and the set of watering events — are owned by the plants and watering capabilities and are READ here, never duplicated.
- Status is a derived (computed) value, not stored state. It is recomputed on read from `intervalDays` + last watering date vs today (Europe/Kiev). "Watered today" naturally resets on the daily calendar boundary because the comparison is against today's calendar date (NFR-USA-03, A5/Q8).
- Notifications/reminders that push to the OS or send messages (FR-REM-08, formerly FR-WATER-07) are Future and intentionally NOT specified here.
- Logging, editing, and deleting watering events themselves are owned by the watering capability (FR-WATER-01..05). This capability only TRIGGERS a watering log via the "water now" action (FR-REM-05) and READS events to derive status; it does not redefine watering-event validation.
- The status thresholds are defined objectively below (see "Status derivation rule") so a tester reading a scenario can decide pass/fail from the dates alone.
- Interactive controls in this capability (the "water now" action on each reminder row) are keyboard-operable and have accessible labels (SC-6, NFR-A11Y-04). The rendered home view's legibility under the «Поливайко» design (urgency colors, status pills, summary card contrast) is verified by a vision pass on the settled screen in Phase 6 (NFR-A11Y-01/02).

### Status derivation rule (used by every scenario below)
Let `today` be the current calendar date in Europe/Kiev, `lastWateredAt` be the date of the plant's latest watering event, and `due = lastWateredAt + intervalDays` (the next calendar date the plant is expected to be watered).

- **overdue** — the plant is past due: `today > due` (today is strictly after the due date). A plant that has NEVER been watered (no watering events) is treated as due and classified **overdue** (the strongest "needs water" signal), so it always surfaces as a reminder.
- **soon** — the plant is due now or imminently: `due` is today (`today == due`) OR `due` is exactly one day after today (`due == today + 1 day`). I.e. soon means "due today or tomorrow" and not yet overdue.
- **healthy** — none of the above: `due` is two or more days after today (`due >= today + 2 days`).

A plant is **"due" (needs watering today)** when its status is **soon OR overdue**. Healthy plants are NOT due. This single definition of "due" is what the summary count (FR-REM-03), the reminder list (FR-REM-04), and the all-done state (FR-REM-06) all use.
...
- **THEN** the plant is not double-counted as still due, its status remains not-due for today, and the row stays in/returns to the done/confirmation state (no raw error or silent failure)
