...
| composite-dish | doing | agent | 8 | food-text, photo-label | US-2 | M3 | Save a logged multi-item dish as ONE named `portion` Food-DB product (macros = components summed in code, re-read from the rows) via a "Save as dish" button → name prompt; re-log by name as a `fact`, qty-scaled (reuse = existing lookup, no new code). No schema change |
