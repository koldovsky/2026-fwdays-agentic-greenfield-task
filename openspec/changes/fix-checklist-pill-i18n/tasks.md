...
- [ ] 1.1 Add `locale?: Locale` to `StatusPillProps` in
...
- [ ] 1.2 Update `src/shared/ui/status-pill/index.ts` barrel if the `StatusPillProps`
...
- [ ] 2.1 Add `locale?: Locale` to `ChecklistRowProps` in
...
- [ ] 2.2 Confirm `src/shared/ui/checklist-row/index.ts` re-exports `ChecklistRowProps`
  so the updated type is visible to callers without reaching into internal files.

## 3. ChecklistPreview: thread locale to rows

- [ ] 3.1 In `src/views/landing/ui/ChecklistPreview.tsx`, pass `locale={locale}` to
...
- [ ] 4.1 In the `StatusPill` test file, add a scenario: render with `status="met"`
...
- [ ] 4.2 In the `ChecklistRow` test file (or create one if absent), add a scenario:
...
- [ ] 4.3 In the `ChecklistPreview` test file (or create one if absent), add a
...
- [ ] 5.1 `yarn lint` passes with no new errors or warnings. Confirm no FSD import
...
- [ ] 5.2 `yarn build` passes. No TypeScript errors on the updated prop interfaces
  or the `t()` call site.
- [ ] 5.3 `yarn test` passes. All pre-existing tests remain green; the new locale
  scenarios from task 4 are also green.

## 6. Independent review (maker != checker)

- [ ] 6.1 Run the `checker` subagent (or `checker-review` skill) on the diff.
...
- [ ] 6.2 Fix any blockers raised by the checker before marking the change complete.
