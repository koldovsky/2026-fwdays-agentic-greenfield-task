...
- [x] 1.1 Create `src/shared/lib/db/migrations/0005_ultra_plan.sql`. Add `'ultra'`
...
- [x] 1.2 Register the new migration file in the migration runner so it executes in
...
- [x] 2.1 `src/entities/subscription/model/types.ts`: add `"ultra"` to the `Plan`
  union. Update the JSDoc comment. Cite FR-BILLING-01.
- [x] 2.2 `src/shared/lib/payments/types.ts`: add `"ultra"` to `PaymentsPlan` and
  `PAYMENTS_PLANS`. `isPaymentsPlan` guard picks it up automatically. Cite
  FR-PAYWALL-02.
- [x] 2.3 `src/shared/lib/db/subscription-repo.ts`: add `"ultra"` to
  `SubscriptionPlan`. No behavior change needed — the repo is generic over the
  plan string.
- [x] 2.4 `src/entities/subscription/lib/subscription.ts`: add `"ultra"` to
...
- [x] 3.1 `src/shared/lib/payments/emulator.ts`: replace the single `PERIOD_DAYS = 30`
...
- [x] 3.2 Update or add emulator unit tests: assert that a `job_hunt_pass`
...
- [x] 3.3 `src/widgets/billing-portal/lib/invoices.ts`: replace the local
...
- [x] 4.1 `src/shared/lib/i18n/types.ts`:
  ...
  - Cite NFR-I18N-01.
- [x] 4.2 `src/shared/lib/i18n/en.ts`: fill all new keys. Use the approved benefit
...
- [x] 4.3 `src/shared/lib/i18n/ua.ts`: mirror 4.2 exactly in Ukrainian. All keys,
  benefit-array lengths, and structural shape must be identical to `en`. Cite
  NFR-I18N-01.
- [x] 4.4 Extend existing i18n parity tests (or add new ones) to cover:
...
- [x] 5.1 `src/views/landing/lib/content.ts`: update `pricingSection()` to assemble
...
- [x] 5.2 `src/views/landing/ui/Pricing.tsx`: rework the layout to:
...
- [x] 5.3 Add or update landing snapshot / rendering tests to assert all four plan
  names appear in the pricing section.

## 6. Upgrade chooser and billing portal

- [x] 6.1 `src/features/upgrade/ui/UpgradePlans.tsx`: update `PLANS` to
...
- [x] 6.2 `src/widgets/billing-portal/ui/BillingPortal.tsx`:
...
- [x] 7.1 `yarn lint` — zero new errors. All new plan-keyed Records must satisfy
  TypeScript exhaustiveness (no missing keys). Record the output.
- [x] 7.2 `yarn build` — zero type errors, zero build errors. Record the output.
- [x] 7.3 `yarn test` — all existing tests pass; all new tests in tasks 2.4, 3.2,
...
- [x] 8.1 Run the `checker` subagent (or `checker-review` skill) against the full
...
- [x] 8.2 Address all BLOCKING findings before merging. Record checker verdict and
