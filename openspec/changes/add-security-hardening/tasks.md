...
- [ ] 1.1 `shared/lib/rate-limit`: pure sliding-window core (`checkRateLimit`) taking an
...
- [ ] 1.2 `shared/lib/rate-limit`: thin in-memory adapter (module-level `Map`, real `Date.now`)
      for route-handler use
- [ ] 1.3 `shared/lib/db/usage-counter-repo.ts`: `get`/`increment` over `usage_counters`
      (already migrated in `add-persistence`); export from `shared/lib/db` barrel

## 2. Wire enforcement into `POST /api/tailor`

- [ ] 2.1 Determine account kind (`currentUserId()` null → `anonymous`, else `free` — stub
      noted in design.md) and request IP before resolving the LLM provider (NFR-SEC-04)
- [ ] 2.2 Anonymous: per-IP rate limiter, window 24h, max `ANON_TAILORING_LIMIT`; over-limit →
      calm `rate_limited` NDJSON error + `status:failed`, no LLM call (NFR-COST-02, NFR-OBS-01)
- [ ] 2.3 Logged-in free: `usage-counter-repo.get` + `canTailor(counter, "free")`; same calm
      rejection path when false
- [ ] 2.4 On a successful run only (`result` event actually yielded): increment the rate-limit
      entry / usage counter; failed runs never consume budget (mirrors FR-TAILOR-03)
- [ ] 2.5 New `TailorErrorCode = "rate_limited"` in `features/run-tailoring/model/types.ts` +
...
- [ ] 3.1 `next.config.ts` `headers()`: CSP (self + `next/font` Google Fonts host + inline
      style for Tailwind), `X-Frame-Options: DENY`, `X-Content-Type-Options: nosniff`,
...
- [ ] 4.1 Hidden honeypot field (`name="website"`, off-screen absolute position, not
      `display:none`, `aria-hidden`, `tabIndex={-1}`) on `TailoringForm`
- [ ] 4.2 Same field on `SignInForm`'s sign-up mode
- [ ] 4.3 Non-empty honeypot on submit → silent no-op (never call `streamTailoring` /
