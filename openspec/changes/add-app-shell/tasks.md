...
- [ ] 1.5 Write a test that an unknown `/plants/[id]` route renders the friendly not-found state with a link back to `/`, never a raw 500 (FR-SHELL-01). Confirm it FAILS.
...
- [ ] 3.1 Run `npm run check:a11y` (axe) on the shell in BOTH light and dark themes; resolve any violations (NFR-A11Y-01).
- [ ] 3.2 Verify WCAG 2.1 AA contrast for text and interactive elements in both themes (NFR-A11Y-02).
- [ ] 3.3 Verify keyboard-only operation: visible focus, accessible names on nav links and theme toggle, Enter/Space activation, and that `<FieldError>` messages are programmatically associated with their fields (NFR-A11Y-04).
- [ ] 3.4 Verify the shell at a 360 px viewport: nav and content visible/operable with no horizontal overflow (NFR-COMPAT-01).
...
- [ ] 4.4 Run `npx openspec validate add-app-shell --strict` — no errors.
- [ ] 4.5 Run `npx openspec validate --all --strict` — no errors.
