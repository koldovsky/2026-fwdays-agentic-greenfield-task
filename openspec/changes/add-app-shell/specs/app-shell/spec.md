...
- **THEN** the action returns a `{ ok: false }` result whose `fieldErrors` or `formError` is rendered as an inline field-level or form-level error message, and the Owner sees no raw 500 page or silent no-op
