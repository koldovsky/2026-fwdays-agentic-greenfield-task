...
contract. Isolation scenarios reuse the slice 001 per-user boundary (FR-AUTH-07) rather than
re-defining it. Scenarios map one-to-one to the acceptance tests the test-engineer will write
test-first (RED) in `backend/tests/test_categories.py`.
