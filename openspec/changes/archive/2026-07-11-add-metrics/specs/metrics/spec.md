...
each requirement below cites its stable id and restates the behavior in SHALL form as the OpenSpec
contract. **Every formula, window, and threshold is fixed by architecture §3 and §4.1 — this contract
references those sections and never re-derives a number.** All metrics are **deterministic pure
functions** over a user's saved `sessions` and `pause_segments` (slice 003) and, for category identity
(`id`/`name`/`color`, including archived), slice 002's `categories` (§7) — computed on read
(architecture §1); "minutes" always means **net** minutes (gross minus pause segments, architecture
§2.2) and "day" always means a calendar day in the user's timezone (§3.7, A-1). Isolation reuses the
slice 001 per-user boundary (FR-AUTH-07) rather than re-defining it. Scenarios map one-to-one to the
acceptance tests the test-engineer will write test-first (RED).
