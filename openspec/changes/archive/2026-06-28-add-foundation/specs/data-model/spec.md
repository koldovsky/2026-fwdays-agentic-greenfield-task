...
`postgresql` datasource whose url is read from `env("DATABASE_URL")`, with a Prisma client
generator. The schema SHALL include a model for each MVP glossary entity — Employee,
