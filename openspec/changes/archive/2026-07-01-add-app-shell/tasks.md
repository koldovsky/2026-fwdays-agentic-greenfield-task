...
- [x] 1.1 `npx expo install @react-navigation/native @react-navigation/native-stack @react-navigation/bottom-tabs react-native-screens react-native-safe-area-context expo-blur`
- [x] 1.2 Wrap the app: `App.tsx` → `SafeAreaProvider` + `ThemeProvider` + `NavigationContainer` (container theme background = `bg`, no white flash)
- [x] 1.3 `src/navigation/` folder per the project-structure rule

## 2. Gated root navigator (FR-SHELL-02)

- [x] 2.1 Root navigator reads `useAuthStore(status)`: `loading` → splash/loader, `signedOut` → auth stack, `signedIn` → tab navigator
- [x] 2.2 Auth stack (native-stack) hosting `AuthScreen`; move the gate out of `App.tsx`
- [x] 2.3 Verify tree-swap: sign-in shows tabs, sign-out returns to auth, no manual reload
...
- [x] 6.1 Mobile `tsc --noEmit` + `eslint .` green; `expo export` bundles clean
