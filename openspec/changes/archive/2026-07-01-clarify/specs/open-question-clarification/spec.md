...
([ADR-0019](../../../../docs/adr/0019-in-memory-open-question-store.md)) — never in the database. Each
entry SHALL hold only what is needed to log on resolution: the resolved-so-far food, the routed
message context, the target date, and an `askedAt` timestamp. The store SHALL NOT retain chat history
or any earlier turns (invariant #1 — the DB is the memory; the model sees only the current message
plus the pending question). At most one Open Question SHALL be pending per user at a time.

#### Scenario: The pending question holds no chat transcript
- **WHEN** an Open Question is stored
- **THEN** the stored record contains only the pending clarification's own data (resolved food,
  routed context, date, timestamp) and no prior conversation turns
...
- **THEN** at most one Open Question remains pending for that user afterward
