...
#### Scenario: Search matches Pokémon outside the first page
- **WHEN** the user types a name that matches a Pokémon not in the first 20 results (e.g. "mewtwo")
- **THEN** matching Pokémon are shown regardless of their position in the full index

#### Scenario: No results shows empty state
- **WHEN** the user types a string that matches no Pokémon name in the full index
