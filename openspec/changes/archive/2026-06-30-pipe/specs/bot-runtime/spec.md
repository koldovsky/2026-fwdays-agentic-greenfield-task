...
LLM call is made in this change. `NOTION_TOKEN` and `NOTION_*` DB ids SHALL be optional. `TZ` SHALL
be optional with a default of `Europe/Kyiv` (used by the later review cron). Secrets SHALL be read
from environment only — never from the repo or the database.
...
- **THEN** the multi-stage Dockerfile builds the image and pushes it to GHCR, and no build step is
  configured to run on the production host
