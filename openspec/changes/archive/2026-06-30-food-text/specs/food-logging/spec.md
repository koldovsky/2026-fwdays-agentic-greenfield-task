...
When the router classifies an inbound message as `log`, the system SHALL resolve the food's
macros, write one `food_log` row scoped to the user, and confirm it. The entry SHALL be recorded
for the date the router resolved (user timezone, including a `вчера`/`yesterday` back-date), not
the message-arrival date when an override is present.
