...
Removes the previous behavior that called a speculative `getAccessToken` JSON-RPC method on the TV (that method is not documented in the Samsung IP Control PDF and does not exist on consumer Tizen TVs — the previous implementation only reached `Connected` under the stub-transport test path).
