...
contract. Isolation scenarios reuse the slice 001 per-user boundary (FR-AUTH-07) and the slice 002
`categories` capability rather than re-defining them. Scenarios map one-to-one to the acceptance
tests the test-engineer will write test-first (RED). Durations are **derived, never stored**
(architecture §2.2); the active timer, its pauses, and undo follow architecture §2.1 and §6.
