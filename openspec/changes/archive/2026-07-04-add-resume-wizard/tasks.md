...
- [x] 5.4 agent-verify: exported PDF/DOCX round-trip check — Ukrainian text renders correctly, footer present/absent matches entitlement default — automated round-trip tests (`resume-pdf.test.ts`, `resume-docx.test.ts`) render a Cyrillic fixture then re-extract it with `pdf-parse`/`mammoth` to prove PT Sans/Calibri actually preserve the glyphs, not just visual inspection
