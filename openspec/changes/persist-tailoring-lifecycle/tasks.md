...
- [ ] 1.1 Author `src/shared/lib/db/migrations/0005_tailoring_lifecycle.sql`:
...
- [ ] 1.2 Add `0005` to the migration runner list (wherever `0004` is registered)
  so `yarn db:migrate` applies it.
- [ ] 1.3 Extend the pglite integration test in
...
- [ ] 2.1 Add `createPending(userId: string, jobDescriptionId: string, jobTitle: string | null): Promise<string>` to `createTailoringRepo` in `src/shared/lib/db/tailoring-repo.ts`.
...
- [ ] 2.2 Add `updateStatus(id: string, status: 'complete' | 'failed', payload?: CompletePayload): Promise<void>` to `createTailoringRepo`.
...
- [ ] 2.3 Refactor `save` to delegate: call `createPending`, then `updateStatus('complete', payload)`,
  returning a `TailoringRecord`. No behavior change; existing call sites keep working.
- [ ] 2.4 Export `createPending`, `updateStatus`, and `CompletePayload` types from
  `src/shared/lib/db/index.ts` alongside the existing exports.
- [ ] 2.5 Unit tests for the new repo methods (extend
...
- [ ] 3.1 Author `src/shared/lib/db/tailoring-cleanup.ts` exporting
...
- [ ] 3.2 Export `markAbandonedPending` from `src/shared/lib/db/index.ts`.
- [ ] 3.3 Unit tests for `markAbandonedPending`: rows older than the TTL are marked
...
- [ ] 4.1 In `src/app/api/tailor/route.ts`: move the free-user `counters.reserve` call
...
- [ ] 4.2 Apply the same timing shift in `src/app/api/tailor/generate/route.ts`.
- [ ] 4.3 Route tests: free user blocked before LLM call if counter exhausted;
...
- [ ] 5.1 In `src/app/api/tailor/route.ts`: after auth + job-description save, call
  ...
  closure.
- [ ] 5.2 In the same route: on terminal `result` event call
...
- [ ] 5.3 Apply the same wiring to `src/app/api/tailor/generate/route.ts`: replace
  the existing post-completion `persistTailoring` call (paid-only) with
  `createPending` at start + `updateStatus` on result/failure.
- [ ] 5.4 Route tests (both routes): logged-in free user — `createPending` called,
...
- [ ] 6.1 In `GET /api/tailoring` (`src/app/api/tailoring/route.ts` or equivalent):
...
- [ ] 6.2 In `GET /api/tailoring/[id]`: remove the paid gate; keep IDOR guard
  (`record.userId !== currentUserId` returns 404, not 403) (NFR-SEC-02).
- [ ] 6.3 Route tests: free logged-in user gets 200 on list and detail; anonymous
  gets 401; IDOR still returns 404; pending/failed rows absent from list.

## 7. i18n

- [ ] 7.1 Add `tailoringStatus` i18n keys to `shared/lib/i18n` types + `ua.ts` +
...
- [ ] 8.1 `yarn lint` — zero warnings or errors.
- [ ] 8.2 `yarn build` — clean compile, no type errors.
- [ ] 8.3 `yarn test` — all existing tests green plus the new tests from tasks
...
- [ ] 9.1 checker subagent (maker != checker): review the diff for IDOR correctness
