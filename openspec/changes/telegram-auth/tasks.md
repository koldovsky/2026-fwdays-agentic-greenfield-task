...
- [ ] 1.2 Create the OTP codes schema file `db/schema/otp_codes.ts` with fields for `id`, `userId`, `code`, `attempts`, `expiresAt`, `isUsed`, and `createdAt`
...
- [ ] 2.1 Create the Telegram Webhook handler at `app/api/telegram/webhook/route.ts` to process incoming messages from Telegram Bot API with token verification using `X-Telegram-Bot-Api-Secret-Token` matching `TELEGRAM_BOT_WEBHOOK_SECRET`
...
- [ ] 2.3 Create registration polling handler at `app/api/auth/poll-registration/route.ts` to check if a pending registration is complete, set the session cookie, and invalidate the registration token from DB immediately after session creation
...
- [ ] 2.5 Create login/OTP route at `app/api/auth/otp/route.ts` with rate-limiting, generic success response for security (user enumeration protection), 3-attempt OTP limit tracking, and session creation logic
