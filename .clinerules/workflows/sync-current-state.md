...
Read and update docs/current-state.md — the live session handoff (last action + timestamp, working-on requirement IDs, next steps, blockers). Use at session start to load context, and before finishing any unit of work.

Invoked as `/sync-current-state`. The argument (if any) scopes the run (a change name, requirement ID, diff ref, or slice name).
