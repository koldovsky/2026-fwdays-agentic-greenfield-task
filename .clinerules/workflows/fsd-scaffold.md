...
Scaffold a new Feature-Sliced Design slice for Vouch at the correct layer with ui/model/api/lib segments and an index.ts public API, honoring the downward-only import rule. Use when adding a feature/widget/entity/view/shared module, or on "create a slice", "add a feature", "scaffold".

Invoked as `/fsd-scaffold`. The argument (if any) scopes the run (a change name, requirement ID, diff ref, or slice name).
