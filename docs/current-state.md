...
<!-- composite-dish (US-2, M3/M4 food track) landed 2026-07-02: save a just-logged MULTI-item dish as ONE named food_database row, per='portion' (one portion = the whole dish, design D1 — no new table/column, YAGNI). A multi-item plate confirmation now carries a `Confirmation.dish` payload (the just-written row ids + localized label) → bot renders a "➕ Save as dish" button (callback `food:savedish:<ids>`, ids joined compactly; OMITTED + console.log note when the payload would blow Telegram's 64-byte callback_data cap — a rare huge dish). Tap → sets a pending free-text `saveDish` Open-Question variant (holds ONLY the row ids + askedAt — NO macros/meal/date: invariant #1, the numbers are re-read from the rows at save; reuses the one-pending-per-chat clarify store + lazy TTL, ADR-0019) → asks a localized name → the next text resolves via the EXISTING pending path (resolveAnswer variant dispatch → saveComposite; no separate public method — dead-code review fix) → NEW src/food/saveDish.ts saveDishToCatalog: re-read the tenant-scoped food_log rows by id (skip missing/foreign), SUM kcal(Int)/protein/fat/carbs IN CODE (invariant #2), find-or-update the user's row by (user_id, name) case-insensitive (a re-save REFRESHES, never duplicates; own-only write so a global-name clash creates the user's own row — invariant #8) → best-effort Notion enqueue (food_database, US-10, never blocks the reply). Reuse is AUTOMATIC via the existing name lookup (resolveFood → fact, reconcileQty scales "2 protein cocktail" → 2 portions) — ZERO new reuse code, ZERO LLM calls in the whole feature (save + reuse are deterministic). An expired saveDish simply DROPS (nothing was logged, nothing to log). NO Prisma migration, NO new dep. Tests +24 (saveDish unit: sum/find-or-update-no-dup/tenant-scoped/missing-skip/empty; confirm: dish payload for >1 rows, none for 1, localized label; service: one portion row summed no-LLM, reuse fact+qty scale, numbers re-read from rows, mirror enqueue, saveDish/expiry variants; bot: button render, 64-byte omission+log, tap sets pending+asks, name reply resolves via resolveAnswer). Gates green (typecheck/lint/format clean; 14 pre-existing Ukrainian-demo RU→UA test failures unchanged, zero new). ADR: the composite = plain portion row (rejected: per-100g composite, and a dishes table + component join) is recorded in the change's design.md D1 (comparison + rejected alternatives) — no separate ADR (docs:check has no ADR trigger for it; matches the shared-* dedup precedent). -->
...
<!-- photo-label (US-3, M4) landed 2026-07-02: nutrition-label photos are first-class (ADR-0024). The vision contract is now LABEL-AWARE + CAPTION-DRIVEN: plateItemSchema gains `fromLabel: boolean` (macros read from a printed КБЖУ/nutrition-facts table in an image); estimatePlate prompt rewritten — the caption is the AUTHORITATIVE item list, one item per caption entry (label→fromLabel=true printed per-100g/ml, plate→visual, text-only→typical), stated amounts (g/ml/tsp/count) converted into qty; text-only items (sugar, black coffee) are NO LONGER dropped. resolvePlate precedence label > Food-DB > visual: a fromLabel item is source=fact with its OWN label macros, foodDbId null, NOT overridden by a catalog match; only non-label names are looked up (still ONE batched query, fully-labelled group issues none). estimatePlate/logPhoto/FoodService.logPhoto now take `images: string[]` (the shared seam already accepted StructuredImage[] — a caller change). refinePlate round-trips fromLabel derived from held source+foodDbId (a fact with no foodDbId = a label fact) so a refined label item stays fact. NEW src/bot/mediaGroup.ts media-group buffer seam: Map<media_group_id,{images,caption,timer}> with an INJECTED debounce scheduler (unit-testable, no real timers) — N photos sharing one Telegram media_group_id buffer → ONE logPhoto over all images → ONE vision call (invariant #5) → one reply; a lone photo (no group) flushes immediately as a one-element array; wired into BotDeps.foodBuffer + index.ts singleton. handlePhoto keeps isProgressPhoto routing PER PHOTO BEFORE buffering (a progress photo never enters the food buffer). Image bytes live only as transient base64 in the buffer + the call, NEVER persisted (invariant #4 — CRITICAL fs-spy test extended to the multi-image run). Real bug fixed: cream/protein/milk labels were logged as ±20-30% estimates and coffee+sugar were dropped. Tests: fromLabel→fact + no catalog override, caption-drives text-only survives, media-group buffer (injected clock), N-image fs-spy, one-vision-call/tenant-scoped, progress bypasses buffer (+mediaGroup unit suite). npm test/typecheck/lint green (pre-existing Ukrainian-demo language failures unrelated). Live vision eval deploy-time (no key, ADR-0013 skip logged). NO Prisma migration, NO new dep, NO enum change. -->
...
| M4 — Vision (photo plate) | 🟡 **food-photo** + **food-photo-ask** + **photo-label** landed (US-3: 1 vision call → multi-item, fact-vs-estimate, one row/item, image never persisted; precision-first plate ask — vision flags one hidden mover, photo-variant Open Question, ONE text-only refine on the answer, expiry logs all as estimate; **photo-label** (ADR-0024): nutrition-label photos = `fact` (extends invariant #3), caption is the authoritative item list — text-only items no longer dropped, and a multi-photo media group collapses to ONE vision call via the `src/bot/mediaGroup.ts` debounce buffer); on-box vision-accuracy eval deferred (needs labeled image set + key) |
...
- Decision records: `docs/adr/` (0001–0024). Latest: **ADR-0024** nutrition-label photos = `fact` +
  caption-drives-item-list + media-group-one-call (extends invariant #3; live vision eval deploy-time).
  **ADR-0023** runtime resilience (crash-and-restart
