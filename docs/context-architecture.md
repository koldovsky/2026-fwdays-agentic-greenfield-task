# Context Architecture — agentic-crash-course
...
- **Static budget:** 4k tokens. **Today:** `AGENTS.md` is the only static doc and
  is within budget (stack-specific sections still TBD until Phase 1).
- **Recently demoted to dynamic:** the full playbook, gates, and portability notes
  live under `.project-factory/` and the `project-factory` skill — loaded on demand,
