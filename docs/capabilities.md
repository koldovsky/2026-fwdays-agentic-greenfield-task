...
| C3   | UPnP TV discovery         | Continuously discover Samsung TVs on the LAN; de-duplicate by `UDN`; mark disappeared TVs offline.                          | FR-DISCOVERY-01…05                                 | C1                      | 1     |
| C4   | Device list UI            | Two-screen SPA's list screen: name, model, IP, connection status per discovered TV.                                         | FR-UI-01…05                                        | C3                      | 1     |
| C5   | TV connection lifecycle   | Establish/tear down IP Control session (per-TV keep-alive HTTPS agent); enforce one-connection-per-TV; auto-reconnect on unexpected disconnect; publish `Connected / Connecting / Disconnected / Offline` state. | FR-CONNECTION-01…04                                | C3                      | 2     |
| C6   | Remote control keys       | Send Samsung IP Control key presses; return success/failure per command; disable UI while disconnected.                     | FR-REMOTE-01…04                                    | C5                      | 3     |
...
- **C5 TV connection lifecycle.** IP Control session state machine per TV: `Disconnected → Connecting → Connected → Disconnected` (plus `Offline` when discovery drops it). One keep-alive HTTPS agent per TV, serialized state-changing commands per TV (no JSON-RPC batching, per the protocol skill). Auto-reconnect with backoff on unexpected close. State published over WebSocket so the UI can gray out controls (FR-REMOTE-04 setup). This is the pivot: everything in Phase 3 sends commands through the session this capability owns.
...
- **C6 Remote control keys.** Backend method `remoteKeyControl` (per the `samsung-ip-control-protocol` skill). One HTTP endpoint per key or one endpoint with a `key` param — decide during proposal. Front-end Orbit `DPad`, transport `IconButton` row, `AppShortcut` row wired up. Disabled state honoured while `!= Connected`.
- **C7 Volume control.** Backend `directVolumeControl` for absolute, `remoteKeyControl` `VOLUME_UP`/`VOLUME_DOWN`/`MUTE` for relative. UI: Orbit `Slider` + mute `IconButton`. Volume-level display (FR-VOLUME-04) only when the TV reports it — otherwise the slider is a write-only control.
