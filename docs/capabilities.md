...
| C11  | Systemd installer         | One Bash script that builds both packages, grants `cap_net_bind_service` to the resolved node binary, writes and enables a systemd unit that runs the back-end as a dedicated `mytv` system user, and starts it. Idempotent — same script is also the update path. Companion `uninstall.sh`. | FR-DEPLOY-01, FR-DEPLOY-02, FR-DEPLOY-03, FR-DEPLOY-04, serves NFR-03, NFR-08 | C1                | 6     |
