...
| C11  | Systemd installer         | `install.sh` grants `cap_net_bind_service` to the resolved node binary, installs production-only runtime deps via `npm ci --omit=dev`, writes and enables a systemd unit that runs the back-end as a dedicated `mytv` system user, and starts it. Refuses if the dists aren't present. Idempotent — same script is also the update path. Companion `uninstall.sh`. | FR-DEPLOY-01, FR-DEPLOY-02, FR-DEPLOY-03, FR-DEPLOY-04, FR-DEPLOY-05, FR-DEPLOY-06, serves NFR-03, NFR-08 | C1                | 6     |
| C12  | Deploy build              | `scripts/build.sh` — no-root Bash script that runs `npm install:all`, `back:build`, and `front:build` and asserts both `dist/index.js` and `dist/index.html` are produced. Runnable on macOS or Linux; artefacts are pure JS. Feeds C11. | FR-BUILD-01                                        | C1                      | 6     |
...
- **C11 Systemd installer.** First Phase-6 capability. Orthogonal to the runtime feature set (C1–C10) — the script consumes their build outputs but changes no runtime behaviour. Adds `scripts/install.sh`, `scripts/uninstall.sh`, and `scripts/mytv.service.tmpl`. `install.sh` refuses to run on non-systemd / non-Linux hosts, then in order: verifies the two dists exist (fails loud pointing at `scripts/build.sh` if not), installs production-only runtime deps via `npm ci --omit=dev` inside `back-end/`, grants `cap_net_bind_service=+ep` to the resolved node binary via `setcap` so an unprivileged user can bind port 80, creates a dedicated `mytv` system user (home `/var/lib/mytv`) if it does not already exist, renders `/etc/systemd/system/mytv.service` from the template with `WorkingDirectory` and `ExecStart` substituted, reloads systemd only if the unit changed, enables the unit (`WantedBy=multi-user.target`), and restarts the service. Supports `--dry-run` for previewing the mutating commands. Idempotent — re-running is the update path. Companion `uninstall.sh` disables + removes the unit but leaves the `mytv` user and `/var/lib/mytv/` alone so a re-install keeps the same UID and preserves the on-disk token store. Serves NFR-03 ("shall run on Orange Pi") and the product principle "easy deployment on low-power hardware."
- **C12 Deploy build.** Second Phase-6 capability, follow-up to C11. Adds `scripts/build.sh` — a no-root, no-systemd Bash script that runs `npm run install:all`, `back:build`, and `front:build` and asserts `back-end/dist/index.js` and `front-end/dist/index.html` afterwards. Runnable on macOS or Linux (build artefacts are pure JS and portable). Feeds C11: the operator either runs both scripts in sequence on the target Pi (`./scripts/build.sh` → `sudo ./scripts/install.sh`), or builds on a dev laptop / CI and `rsync`s only `back-end/dist/`, `back-end/package.json`, `back-end/package-lock.json`, `front-end/dist/`, and `scripts/` to the target. Splitting build off `install.sh` removed the dev toolchain (TypeScript, Vite, vitest, bats, eslint) from the target's dependency footprint and made every install reproducible against a specific `dist/` artefact set.
