...
- Principal implementation decisions:
  ...
  - consolidated the public API into Nitro routes under `routes/api/*`, retired the root `/api` function entries, explicitly enabled root Nitro scanning with `serverDir: "./"` so the Vercel preset actually registers those handlers, and kept the preview-only Phase 0 probe server-only while the exact deployed function count remains provisional until human Preview verification;
  ...
  - recorded the human local Phase 4 verification pass, including the Nitro build, the Nitro-owned public API route inventory, the Vercel-preset parity build topology of one `__server` SSR function plus four API functions, and the provisional deployment footprint that still awaits Preview confirmation.
- Deterministic checks run by Codex:
  ...
  - a later dependency interoperability correction pinned Nitro's transitive `nf3` resolution to `0.3.21` with a narrow npm override after `nf3@0.3.18` reproduced a Nitro externals failure against bundled CommonJS `@vercel/nft@1.10.2`; lint, typecheck, Phase 3, Phase 4, default build, deterministic tests, Vercel-preset build, and Vercel output verification passed after the correction, while Preview remains previously passed and Production remains pending
