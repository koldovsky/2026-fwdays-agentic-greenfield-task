...
- explicitly enabled Nitro filesystem scanning from the project root with `serverDir: "./"` so the root `routes/api/*` handlers are actually registered by the Vercel preset
...
A human then ran the complete local verification in normal PowerShell and `npm run verify:phase4` passed in full. The human local wrapper verified lint with 0 errors and 6 pre-existing warnings, typecheck, all 109 deterministic tests, Phase 0-4 suites including Phase 4 at 15/15, the Vite client build, the Vite SSR build, the Nitro build, and the final Phase 4 artifact plus sensitive-boundary verification. The final local footprint is a Nitro server runtime owning the SSR route and the four public API routes under `routes/api/*`, and the preview-only Phase 0 probe remains server-only and excluded from the deployed count. The Vercel-preset parity build currently emits five functions total: one `__server` SSR function plus four API functions under `routes/api/*`; the exact deployed Vercel function count remains provisional until human Preview verification.
...
The dependency interoperability correction also held in the latest offline pass. A clean build reproduced a Nitro externals failure after the client and SSR builds completed because `nf3@0.3.18` imported the CommonJS `@vercel/nft@1.10.2` package through a named ESM import for `nodeFileTrace`. The repository now uses a narrow npm override so Nitro resolves `nf3@0.3.21`, whose published trace chunk imports the same `@vercel/nft` package through the default CommonJS interop object before reading `nodeFileTrace`. This changed only package resolution and did not change Nitro routes, `serverDir`, public API contracts, provider integration, Redis, sessions, polling, storage, deployment behavior, or UI behavior.

Verification after the dependency correction passed for `npm run lint`, `npm run typecheck`, `npm run test:phase3`, `npm run test:phase4`, `npm run build`, `npm run test:deterministic`, and `git diff --check`. With process-level `NITRO_PRESET=vercel`, `npm run build` also passed and `node --experimental-transform-types ./scripts/verify-phase4-vercel-output.ts` verified the expected one SSR function plus four API functions. Preview verification remains previously passed, and Production configuration plus Production verification remain pending.
...
The first live deployment was accidentally classified as Production, returned `404: NOT_FOUND` at the root, and the health-only smoke response exceeded 65,536 bytes. No inbox was generated and no live Emailnator request occurred. That live evidence disproved the earlier no-Nitro assumption; the first consolidation had placed the public API adapters in TanStack route-tree files under `src/routes/api/*`, and the subsequent fix had to do two things: move the handlers to the Nitro filesystem `routes/api/*` tree and explicitly enable root scanning with `serverDir: "./"` so the Vercel preset would register those files. The Vercel-preset parity build now confirms the provider output topology as five functions total: one `__server` SSR function plus four API functions under `routes/api/*`. The legacy root API entries were retired, the probe implementation remains server-only in `server/providers/emailnator/probe.server.ts`, and the deployed Vercel footprint remains provisional until a human Preview deployment confirms it.
